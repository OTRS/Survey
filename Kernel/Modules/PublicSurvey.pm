...
sub Run {
    ...
    if ( $Self->{Subaction} eq 'PublicSurveyVote' ) {
        ...
        my %Survey = $SurveyObject->PublicSurveyGet( PublicSurveyKey => $PublicSurveyKey );
        if ( $Survey{SurveyID} ) {
            @QuestionList = $SurveyObject->QuestionList( SurveyID => $Survey{SurveyID} );
            ...
            if ( !scalar keys %Errors ) {
                ...
                $SurveyObject->PublicSurveyInvalidSet( PublicSurveyKey => $PublicSurveyKey );
                $Output = $LayoutObject->CustomerHeader( Title => 'Survey' );
    ...
    # ------------------------------------------------------------ #
        ...
        $Output = $LayoutObject->CustomerHeader( Title => 'Survey Vote' );
        ...
        my @QuestionList = $SurveyObject->QuestionList( SurveyID => $SurveyID );
        for my $Question (@QuestionList) {
            ...
            if ( $Question->{Type} eq 'Radio' || $Question->{Type} eq 'Checkbox' ) {
                ...
                for my $Row (@AnswerList) {
                    my %Answer = $SurveyObject->AnswerGet( AnswerID => $Row->{VoteValue} );
                    my %Data;
                ...
                if ( $Question->{Type} eq 'Textarea' && $Data{Answer} ) {
                    ...
                    if ($1) {
                        $Data{Answer} =
                            $Kernel::OM->Get('Kernel::System::HTMLUtils')->ToAscii( String => $Data{Answer} );
                    }
                }
                push( @Answers, \%Data );
            }
    ...
    if ($UsedSurveyKey) {
        ...
        for my $Question (@QuestionList) {
            ...
            $LayoutObject->Block( Name => 'PublicQuestions' );
